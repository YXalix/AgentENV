...
/// namespaces and the scoped capability set, right before `execve`. This is
/// the only way to hand the child a descriptor that must never exist in the
/// server process — for example a TAP queue attached inside the sandbox
/// network namespace — because a descriptor opened in the server (even for an
/// instant without `FD_CLOEXEC`) can race into unrelated concurrent spawns.
/// Registering a hook forfeits `posix_spawn` eligibility for this spawn; leave
/// it `None` for spawns that do not need it.
