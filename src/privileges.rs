...
///
/// The handle owns the pid and is the child's only reaper. Dropping it keeps
/// the tokio `kill_on_drop(true)` contract this replaces: the child is
...
// symbol. Passing it makes the child inherit the current environment, like
// the tokio spawn path this replaces.
extern "C" {
...
fn posix_spawn_exec(spec: &ScopedSpawnSpec) -> io::Result<ScopedChild> {
    ...
    // Match the fork+exec child setup: caught signals reset to their default
    // disposition and SIGPIPE restored to SIG_DFL; the signal mask inherits.
...
mod tests {
    ...
    async fn scoped_spawn_hands_fd_to_child_via_file_action() -> Result<()> {
        ...
        // the same contract the pre-exec hook used to satisfy for the TAP
        // queue.
