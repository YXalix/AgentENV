...
>
> - AgentENV：`/root/github/AgentENV`，分支 `feat-tap-v3`，提交 `36cfebc`
>   （后续强化：`da1c837` pidfd 收割；本提交 handoff 借用化 + `activate`
>   收敛）
...
1. `TUNSETIFF`、`TUNSETVNETHDRSZ`、`TUNSETOFFLOAD` 每 slot 一生一次
   （netns 创建时 attach、预设 vnet header 与 offload）；
...
账单：5 → **0**（TUNSETOFFLOAD 一并移到 attach 时预设；activate 按 guest
协商命中预设即跳过，非标协商的 guest 回退为 activate 时下发一次，见 §5）。

实测动机（tap-rtnl.bt，resume 风暴，内核 6.6）：activate 路径的
TUNSETOFFLOAD p50 ≈ 0.5–2ms，约 15% 的调用 > 128ms，最大 291ms；同时
rtnl 持锁时长仅 µs–8ms —— 尾巴来自大量短临界区下的排队/唤醒滞后而非
长持锁者，因此把最后一个热路径 tun ioctl 整体移出 resume 路径。
...
- Firecracker 侧（`3266cc03c`）：`Tap::from_preconfigured_fd` =
  ...
  `activate()` 的 `TUNSETOFFLOAD` 在协商命中预设时跳过（§5）。
...
2. 常量：`TUNSETIFF`、`TUNSETVNETHDRSZ`（`_IOW('T', 216, int)`，内核经
   userspace 指针读长度，故传引用）、`TUNSETOFFLOAD`（`_IOW('T', 208,
   c_uint)`，内核从参数值本身取标志，故**传值**）、
   `IFF_TAP|IFF_NO_PI|IFF_VNET_HDR`、`FC_VNET_HDR_LEN = 12`（与
   Firecracker `vnet_hdr_len()` 配对）、`FC_TAP_OFFLOAD_PRESET =
   TUN_F_CSUM|TSO4|TSO6|TUN_F_UFO = 0x17`（与 Firecracker
   `LAUNCHER_PRESET_OFFLOAD` 配对）。
...
4. `attach_tap_queue(tap_name)`：`O_RDWR|O_NONBLOCK|O_CLOEXEC` 打开
   `/dev/net/tun`，`TUNSETIFF` 挂队列后**无条件** `TUNSETVNETHDRSZ(12)` 与
   `TUNSETOFFLOAD(0x17)`——预设（vnet header + offload）是 `fdp:` 契约
   的核心，不是可选行为。
...
- `activate()` 的 `TUNSETOFFLOAD` 改为**条件跳过**（本仓库配套的后续
  FC 提交）：`LAUNCHER_PRESET_OFFLOAD = TUN_F_CSUM|TSO4|TSO6|TUN_F_UFO`
  （与 AgentENV `FC_TAP_OFFLOAD_PRESET` 配对）；当且仅当"fdp 预设队列（
  `vnet_hdr_size_preset`）且 `build_tap_offload_features(acked_features)`
  恰等于该预设"时跳过 ioctl，其余情况照旧下发协商值——非标 guest 的
  协商在两个方向上都会被纠正，最坏退化是 offload 停在 0、host 软件分段
  （慢而不断），不会出现"预设偏大 → host 大帧 → guest 小缓冲 → 静默
  丢弃"的黑洞。跳过判定收敛在 `skips_tap_offload_ioctl`，配套
  `test_tap_offload_preset_skip`（预设值与标准 guest 协商的映射一致性 +
  四个跳过/回退分支）。
...
```text
AgentENV     36cfebc   feat(sandbox): hand Firecracker a slot-owned,
                      pre-configured TAP queue
Firecracker  3266cc03c feat(net): accept pre-configured TAP queues via
                      fdp: host_dev_name
AgentENV     8b102f5   feat(sandbox): preset TAP offload at slot attach
Firecracker  18f310b5b feat(net): skip TUNSETOFFLOAD when the launcher
                      preset matches the guest negotiation
```
...
- offload 跳过仅由 FC 侧条件判定，两仓库四个组合均安全：
  - 新 AgentENV（预设 0x17）+ 新 FC（条件跳过）：热路径 0 次 tun ioctl；
  - 新 AgentENV + 旧 FC：activate 照旧下发协商值，预设被等值覆盖，无害；
  - 旧 AgentENV（fdp 但无预设）+ 新 FC：跳过后 offload 停在设备默认 0，
    guest GSO 帧 host 软件分段，**性能退化、不破坏正确性**；
- bundle（`config/deps_manifest.toml`）升级到含 `3266cc03c` 与跳过提交的
  构建之前，保持 `preopen_tap = false`。
...
5. offload 预设回归：resume 后 guest 内大流量传输（如 iperf）确认吞吐
   无台阶式下降——命中预设（跳过 ioctl）时行为应与 FC 主动下发 0x17
   完全一致；同时确认 `@io_cnt[TUNSETOFFLOAD, firecracker] == 0`。
...
| TUNSETOFFLOAD | 100 | **0**（attach 时每 slot 一次；activate 命中预设跳过） |
| rtnl wait 直方图 | 基线 | 对比 |

实测（内核 6.6，resume 风暴）曾测得 activate 路径 TUNSETOFFLOAD p50 ≈
0.5–2ms、约 15% 调用 > 128ms、最大 291ms，而同期 rtnl 持锁仅 µs–8ms：
尾巴源于大量短临界区下的排队/唤醒滞后（convoy），不是长持锁者，故将
该 ioctl 整体移出热路径而非优化其单次成本。移出后 resume/pause 全路径
0 个 rtnl 操作；`@rtnl_wait_us` 的尾巴预计**不会**因此消失（持锁来源是
slot 补充等冷路径搅动），那是另一个工单（见 §9）。
...
- 无持久化状态迁移：fd 不跨进程持久，回滚 = revert 对应 AgentENV 提交即可
